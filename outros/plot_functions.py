...
def plot_kd_nxm(qaa_data, kd_qaa, kd_campo, n_rows, n_cols, dir_o, formato):
    ...
    fig.text(0.5, 0.01, 'Comprimento de Onda [nm]', ha = 'center', fontsize = 8)
    fig.text(0.01, 0.5, 'Coef. de Atenuação de Ed (Kd) [m-1]', va = 'center', rotation = 'vertical', fontsize = 8)
    fig.text(0.5, 0.97, label_campo + ' Campo x ' + label_qaa + ' QAA' + ' (Lb0 = ' + str(qaa_data['dados']['lb0']) + ')', ha = 'center', fontsize = 10)
    ...
    for i in qaa_data[kd_qaa]:
        ...
        plt.xlim(400, 700)
        plt.ylim(0, 2.5)
        plt.title(i, fontsize = 6, va = 'top')
        
        if n == 1:
            plt.yticks([0.0, 0.5, 1.0, 1.5, 2.0, 2.5], fontsize = 6)
            plt.xticks([])
            ...
            plt.xticks([400, 500, 600, 700], fontsize = 6)
            plt.yticks([])
